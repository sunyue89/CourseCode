...
//Homework 3 graph constructor
graph::graph(int graphid, vector<int> &data) {
	...
	for (int i = 1; i < data.size(); i = i + 3) {
		Nodes[data[i]]->AddEdge(Nodes[data[i + 1]], data[i + 2]); //initilize nodes with connected nodes and edge cost, data must be in provided format
	}
}
